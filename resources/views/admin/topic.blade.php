                        ...
                        <tbody>
                            @foreach ($packages as $i => $package)
                                <tr>
                                    <th>{{ $i + 1 }}</th>
                                    <td>{{ $package->title }}</td>
                                    <td>{{ $package->topic_type }}</td>
                                    <td>
                                        <a href="/admin/topic/{{ $package->slug }}"
                                            class="btn btn-sm bg-green-500 border-none">Edit</a>
                                        <button onclick="delete_package('{{ $package->slug }}')"
        ...
        function delete_package(slug) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = `/admin/topic/${slug}/delete`
                }
            })
        }
    </script>
