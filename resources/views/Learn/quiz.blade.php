...
<body x-data="main_data" class="relative">
    ...
    <div id="capture"
        ...
        @if ($package->timer > 0 && session($package->slug)['is_finished'] == false)
            <div>
                                ...
                                @if ($item->a)
                                    <div class="choice flex items-center mb-4">
                                        <input type="radio" id="a-{{ $item->id }}"
                                            ...
                                            @disabled(session($package->slug)['is_finished'] == true)>
                                        <label
                                ...
                                <div class="mt-3">
                                    @if (
                                        $package->show_correction_quiz == 1 &&
                                            ...
                                            session($package->slug)['is_finished'] == true)
                                        @if ($item->result == true)
                                            <div>
                                                <div id="{{ $i }}-alert"
    ...
    <script>
        async function submit_jawaban(jawaban) {
            let responses = await fetch("/api/submit-jawaban/quiz", {
                method: 'POST',
                headers: {
            ...
            Alpine.data('main_data', () => ({
                ...
                async next() {
                    ...
                    if (next_page <= App.page_total) {
                        ...
                        @if (session($package->slug)['is_finished'] == false)
